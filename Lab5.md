...
lab5 <- read.csv("C:/Users/User/Documents/GitHub/Lab5/lab5data.csv")
```
...
The hypothesis is not supported, since the difference between the GPA of
Greek women and non-Greek women is not significant (p = .38).
...
The hypothesis is not supported, since the difference between the GPA of
men and women is generally not significant (p = .73). \# Q3: Create a
bar graph to compare GPA by gender and greek (either graph works)

``` r
lab5_clean <- lab5 %>% drop_na(GPA)

plot1<-summarySE(lab5_clean, measurevar="GPA", groupvars=c("Group"))

ggplot(plot1, aes(x = Group, y = GPA, fill = Group)) +
  geom_col()  + theme_bruce() + theme(axis.text.x = element_text(angle = -10))
```
